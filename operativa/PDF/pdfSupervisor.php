...
//SACAR EL NUMERO DE SEMANA A PARTIR DE LA FECHA
  ...
  //sacar sus recursos para ese dia y ese turno
  $recursosCon = $recursos -> ModSupervisores($actCon['id'], $_GET['fecha'], $turno);
  if ($recursosCon == null || $recursosCon == false) {
    $recursosCon = $recursos -> RecursosSupervisores($actCon['id'], $_GET['fecha'], $turno);
  }
  //comprobamos que la actividad tiene recursos.
  if ($recursosCon!=null && $recursosCon != false && $recursosCon[$turno]>0) {
    //comprobamos que tiene a gente asignada para ese dia y ese turno.
    $listaEmpleados = $personal -> empleadosServicio($actCon['id'], $_GET['fecha'], $turno);
    //sacamos las furgonetas en las que se dividen los conductores.
    $listaFurgos = $personal -> furgonetas($_GET['fecha'], $turno, $actCon['id']);
    foreach ($listaFurgos as $furgonetas) {
      //sacamos la informacion para esa furgoneta ese dia como si fuese una act. individual
      $empleadosFurgo = $personal -> empleadosFurgo($actCon['id'], $_GET['fecha'], $turno, $furgonetas['furgo']);
      if ($empleadosFurgo!=null && $empleadosFurgo!=false && count($empleadosFurgo)>0) {
        if ($i<=1) {
             $der=40;
        }
          //bucle para recoger array de nombre
          $numeroPersonas = count($empleadosFurgo);
          $descripcion = $furgonetas['furgo'] ." - " .$numeroPersonas;
         if($izq>$der){
           $derNombre=0;
           $pdf->SetY($der);
           ...
           //sacar la lista de empleados para esa actividad, dia y turno.
             foreach ($empleadosFurgo as $empleado) {
