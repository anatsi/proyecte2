...
->setCellValue('A1', 'VIN')
->setCellValue('B1', 'DERECHA')
->setCellValue('C1', 'IZQUIERDA')
...
if (isset($_GET['b'])) {
  ...
  $lista=$puertas->listaPuertaFiltrados($filtro);
}else {
...
foreach ($lista as $puerta) {
  ...
  $objPHPExcel->setActiveSheetIndex(0)
  ->setCellValue('A'.$i, $puerta['bastidor'])
  ->setCellValue('B'.$i, $puerta['derecha'])
  ->setCellValue('C'.$i, $puerta['izquierda'])
  ->setCellValue('D'.$i, PHPExcel_Shared_Date::PHPToExcel( $puerta['fecha'] ))
  ->setCellValue('E'.$i, $puerta['hora'])
  ->setCellValue('F'.$i, $puerta['usuario'])
  ;
