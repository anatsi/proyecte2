...
class Movimientos extends dbMovimientos
{
  ...
  function listaMovimientosFiltrados($b){
    //Construimos la consulta
    $sql="SELECT * from movimientos WHERE concat(bastidor, origen, fecha_origen, hora_origen, destino, fecha_destino, hora_destino, usuario, rol, error, lanzamiento) LIKE '%".$b."%' ORDER BY id desc";
    //Realizamos la consulta
    $resultado=$this->realizarConsulta($sql);
    if($resultado!=null){
      //Montamos la tabla de resultados
  ...
  function cuentalistaMovimientosFiltrados($b){
    //Construimos la consulta
    $sql="SELECT count(*) as 'recuento' from movimientos WHERE concat(bastidor, origen, fecha_origen, hora_origen, destino, fecha_destino, hora_destino, usuario, rol, error, lanzamiento) LIKE '%".$b."%'";
    //Realizamos la consulta
    $resultado=$this->realizarConsulta($sql);
    if($resultado!=null){
      //Montamos la tabla de resultados
