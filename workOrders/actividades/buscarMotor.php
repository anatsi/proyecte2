<!DOCTYPE html>
<html>
  <head>
    <meta charset="utf-8">
    <title></title>
    <style media="screen">
      tr:nth-child(even) {
        background-color: #CAC6C5;
      }
    </style>
    <!--Script para fijar la cabecera de las tablas-->
    <script type="text/javascript" src="../../js/jquery.stickytableheaders.min.js"></script>
    <script type="text/javascript">
      $(function() {
        $("table").stickyTableHeaders();
      });
    </script>
  </head>
  <body>
    <?php
    require_once '../bbdd/motor.php';
    $motor= new Motor();

    $b = str_replace(" ", "%", $_POST['b']);

    if(!empty($b)) {
          buscar($b);
    }else{

      $lista= $motor->listaMotor();
      $recuentos = $motor -> cuentaListaMotor();
        echo "
          <h3>
          <a href='excelMotor.php' title='Exportar todo a excel'><img src='data:image/png;base64,'></a>
          <img onclick='location.reload(true);' src='data:image/png;base64,'>
          </h3>

          <br>TOTAL: " .$recuentos[0]['recuento']."
          <table id='tablamod'>
          <thead id='theadmod'>
            <tr id='trmod'>
              <th scope='col' id='thmod'>VIN</th>
              <th scope='col' id='thmod'>INFO</th>
            </tr>
          </thead><tbody id='tbodymod'>

          "; foreach ($lista as $motores) {
             echo "
                <tr id='trmod'>
                  <td data-label='VIN' id='tdmod'>".$motores['bastidor']."</td>
                  <td data-label='INFO' id='tdmod'>".$motores['motor']."</td>
                </tr>

          ";} echo "</tbody></table></div>";
    }

    function buscar($b) {
      require_once '../bbdd/motor.php';
      $motor= new Motor();
        $filtrados= $motor->listaMotorFiltro($b);
        $filtro=base64_encode($b);
        $recuentos = $motor -> cuentaListaMotorFiltro($b);
            echo "
                <h3><a href='excelMotor.php?b=".$filtro."' title='Exportar filtrados a excel'><img src='data:image/png;base64,'></a>
                <img onclick='location.reload(true);' src='data:image/png;base64,'>
                </h3>

                <br>TOTAL: " .$recuentos[0]['recuento']."
              <table id='tablamod'>
              <thead id='theadmod'>
                <tr id='trmod'>
                  <th scope='col' id='thmod'>VIN</th>
                  <th scope='col' id='thmod'>INFO</th>
                </tr>
              </thead><tbody id='tbodymod'>

              "; foreach ($filtrados as $motores) {
                 echo "
                    <tr id='trmod'>
                      <td data-label='VIN' id='tdmod'>".$motores['bastidor']."</td>
                      <td data-label='INFO' id='tdmod'>".$motores['motor']."</td>
                    </tr>

              ";} echo "</tbody></table></div>";

    }
     ?>

     <!--ORDENAR TABLA
     <script type="text/javascript" src="../js/jquery.min.js"></script>-->
     <script type="text/javascript" src="http://cdnjs.cloudflare.com/ajax/libs/jquery.tablesorter/2.9.1/jquery.tablesorter.min.js"></script>
     <script>
     $(function(){
       $("#tablamod").tablesorter();
     });
   </script>
  </body>
</html>
