...
if (isset($_GET['b'])) {
  ...
  $lista=$clatters->listaClatterFiltrados($filtro);
}else {
...
foreach ($lista as $clatter) {
  ...
  $objPHPExcel->setActiveSheetIndex(0)
  ->setCellValue('A'.$i, $clatter['bastidor'])
  ->setCellValue('B'.$i, $clatter['clatter'])
  ->setCellValue('C'.$i, PHPExcel_Shared_Date::PHPToExcel( $clatter['fecha'] ))
  ->setCellValue('D'.$i, $clatter['hora'])
  ->setCellValue('E'.$i, $clatter['usuario'])
  ;
